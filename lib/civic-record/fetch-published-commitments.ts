...
import { COMMITMENT_DATABASE_ENV, COMMITMENT_PILOT_DATABASE_ID } from "./notion-pilot-ids";
...
function databaseId(): string {
  ...
  return fromEnv || COMMITMENT_PILOT_DATABASE_ID.replace(/-/g, "");
}
...
function mapPage(page: any): PublicCommitment | null {
  ...
  const publicationStatus = sel(props, "Publication Status");
  ...
  if (!commitmentText || !sourceUrl) return null;
  return {
...
export async function getPublishedCommitments(): Promise<{ commitments: PublicCommitment[]; total: number; byDuty: Record<string, number>; truncated: boolean; configured: boolean }> {
  if (!process.env.NOTION_TOKEN) return { commitments: [], total: 0, byDuty: {}, truncated: false, configured: false };
  ...
  return { commitments, total: commitments.length, byDuty, truncated, configured: true };
}
