  ...
  return (
    <div className="mx-auto min-h-screen max-w-2xl">
      ...
      <main className="px-4 py-8">
        ...
        {error ? (
          <div className="mt-4 border-y border-forest-500/15 bg-forest-50 px-4 py-6 text-center">
            ...
            {!loading && forState.length === 0 ? (
              <div className="mt-8 border border-dashed border-forest-500/20 py-10 text-center">
                ...
                <p className="mt-2 text-xs text-forest-500">
                  When citizens submit and ISEYC publishes, demands will group here by duty — so communities can see patterns, not personalities.
                </p>
                <Link href="/" className="mt-4 inline-block text-sm font-semibold text-forest-700 underline underline-offset-2">
                  Submit a mandate →
                </Link>
              </div>
            ) : (
                  ...
                  return (
                    <section key={d.id}>
                      ...
                      <ul className="space-y-3">
                        {items.map((v) => (
                          <li key={v.id}>
                            <Link href={`/mandate/${v.id}`} className="block border border-forest-500/12 bg-white px-3 py-3 transition hover:border-forest-500/30">
        ...
        <div className="mt-10 border-t border-forest-500/10 pt-6 text-xs leading-relaxed text-forest-500">
          ...
          <p className="mt-3">Counts describe published demands only. They are not votes, polls, or popularity scores.</p>
        </div>

        <p className="mt-8 text-center">
          <Link href="/" className="text-sm font-semibold text-forest-600 underline underline-offset-2">← Back to Civic Mandate</Link>
        </p>
      </main>
      <Footer />
    </div>
